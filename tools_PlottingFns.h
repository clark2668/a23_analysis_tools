...
vector<TGraph*> makePaddedGraphs(vector<TGraph*> graphsIn, bool isSoftTrigger, string xlabel, string ylabel, vector<string> titles){
	...
	if (isSoftTrigger == false){ nPointsNew = 2048;} else {nPointsNew = 2048;}
	...
	for (int i = 0; i < graphsIn.size(); i++){
		...
		if (isSoftTrigger == false && graphSize > 2048)
			{ graphSize = 2048;}   
		if (isSoftTrigger == true && graphSize > 2048) 
			{graphSize = 2048;}    
...
string getRunSummaryFilename(int stationID, char* outputdir, char* runfile ){
	...
	int runNum = getrunNum(runfile);
	ss.str("");
	// ss << outputdir << "RunSummary/run_summary_station_" << stationID << "_run_" << runNum << ".root"; //carl's original
