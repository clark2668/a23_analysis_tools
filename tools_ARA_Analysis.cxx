...
//Custom includes
#include "tools_ARA_Analysis.h"
#include "tools_Constants.h"

using namespace std;

ARA_Analysis::ARA_Analysis()
{
  //Default constructor
}


ARA_Analysis::ARA_Analysis(int stationID)
{
  Pairs = setupPairs(stationID);
  Faces = setupFaces(stationID);
  setupCorrelationPairs(stationID, Pairs_for_corr_V, Pairs_for_corr_H);
}

void ARA_Analysis::getMaximum(TGraph * plot, double &x_max, double &y_max){
...
void ARA_Analysis::getAbsMaximum( TGraph *plot, double &x_max, double &y_max )
{
  int nPoints = plot->GetN();
...
double ARA_Analysis::getRMS(double *array, int bin) {
...
double ARA_Analysis::getRMS( TGraph *plot, int numPointsToInclude)
{
  int nPoints = plot->GetN();
...
double ARA_Analysis::getWaveformVariance(double *array, int bin) {
...
double ARA_Analysis::getWaveformVariance( TGraph *plot, int numPointsToInclude)
{
  int nPoints = plot->GetN();
  //  Double_t *xVals = plot->GetX();                                                                                                          
  Double_t *yVals = plot->GetY();
...
double ARA_Analysis::integrateBinPower( TGraph *plot, int numBinsToIntegrate, vector<double> &integratedBins)
{
  int nPoints = plot->GetN();
...
double ARA_Analysis::integrateAllPower(TGraph *plot){
  int nPoints = plot->GetN();
  double sum = 0.;
...
double ARA_Analysis::getAveragePower(TGraph *plot){
  int nPoints = plot->GetN();
  double average = integrateAllPower(plot)/nPoints;
  return average;
}

vector<int> ARA_Analysis::getDividers(TGraph *plot, int divisions){
  int nPoints = plot->GetN();
  vector<int> dividers;
...
double ARA_Analysis::integrate(TGraph *plot, int firstBin, int lastBin){
  int nPoints = plot->GetN();
  if (lastBin > nPoints){
...
double ARA_Analysis::getAverage(TGraph *plot, int firstBin, int lastBin){
  double average = integrate(plot, firstBin, lastBin)/(double)(lastBin-firstBin);
  return average;  
}

double ARA_Analysis::getDeviation(TGraph *plot, int firstBin, int lastBin, double average){
...
void ARA_Analysis::getAverageDeviation_Divided(TGraph *plot, int divisions, double &average_out, double &deviation_out){
...
void ARA_Analysis::getVAveragesDeviations_Divided(vector<TGraph*> vPlots, int divisions, vector<double> &vAverages, vector<double> &vDeviations){
...
double ARA_Analysis::integratePowerFirstNBins(TGraph *plot, int nBins){
  int nPoints = plot->GetN();
  if (nPoints < nBins){
    nBins = nPoints;
  }
...
double ARA_Analysis::getAveragePowerFirstNBins(TGraph *plot, int nBins){
  int nPoints = plot->GetN();
  if (nPoints < nBins){
    nBins = nPoints;
  }
...
vector<TGraph*> ARA_Analysis::makeIntegratedBinPowerGraphs(vector<TGraph*> graphsInput, int numBinsToIntegrate, string xlabel, string ylabel, vector<string> titles){
...
vector<TGraph*> ARA_Analysis::makeAvgVoltageFromIntPower(vector<TGraph*> graphsInput, int numBinsToIntegrate, string xlabel, string ylabel, vector<string> titles){
...
void ARA_Analysis::getAbsMaximum(vector<TGraph*> graphs, vector<double> &xs, vector<double> &ys ){
...
void ARA_Analysis::getRMS(vector<TGraph*> graphs, vector<double> &vRMS, int numPointsToInclude){
...
void ARA_Analysis::getWaveformVarianceSamples(vector<TGraph*> graphs, int numPointsToInclude, vector<double> &vVariance, vector<double> &vSamples){
...
void ARA_Analysis::addVarianceSamples(vector<double> vWaveformVariance, vector<int> vSamples, vector<double> vWaveformVarianceTotal, vector<long int> vSamplesTotal)
{
...
vector<double> ARA_Analysis::getTotalPowers(vector<TGraph*> graphs){
...
vector<double> ARA_Analysis::getAveragePowers(vector<TGraph*> graphs){
  vector<double> powers;
  double power;
  for (int i = 0; i < graphs.size(); i++){
...
vector<double> ARA_Analysis::getAveragePowersFirstNBins(vector<TGraph*> graphs, int nBins){
  vector<double> powers;
  double power;
  for (int i = 0; i < graphs.size(); i++){
...
vector<vector<vector<vector<int> > > > ARA_Analysis::setupPairs(int stationId){
...
vector<vector<vector<double> > > ARA_Analysis::getHitTimeDifferences(vector<double> hitTimes, vector<double> peakIntPowers, vector<vector<vector<vector<int> > > > Pairs){
...
vector<vector<vector<double> > > ARA_Analysis::getHitTimeWeight_LesserPower(vector<double> hitTimes, vector<double> peakIntPowers, vector<vector<vector<vector<int> > > > Pairs){
...
vector<vector<vector<double> > > ARA_Analysis::getHitTimeWeight_LesserFractionalPower(vector<double> hitTimes, vector<double> peakIntPowers, vector<double> totalPowers, vector<vector<vector<vector<int> > > > Pairs){
...
vector<vector<vector<double> > > ARA_Analysis::getHitTimeWeight_LesserRelativePower(vector<double> hitTimes, vector<double> peakIntPowers, vector<double> averagePowers, vector<vector<vector<vector<int> > > > Pairs){
...
vector<vector<vector<double> > > ARA_Analysis::getHitTimeWeight_LesserRelativeRMS(vector<double> hitTimes, vector<double> peakIntRMS, vector<double> waveformRMS, vector<vector<vector<vector<int> > > > Pairs){
...
vector<TH1D*> ARA_Analysis::makeHitTimeDist(vector<vector<vector<double> > > delays, int polarization){
...
double ARA_Analysis::getMean(vector<double> v){
...
double ARA_Analysis::getRms(vector<double> v){
...
vector<vector<double> > ARA_Analysis::getRmsDelays(vector<vector<vector<double> > > delays){
...
double ARA_Analysis::getRmsPol(vector<vector<vector<double> > > delays, int polarization){
...
double ARA_Analysis::getWeightedSum(vector<double> v, vector<double> weights){
...
double ARA_Analysis::getWeightedMean(vector<double> v, vector<double> weights){
  if (v.size() != weights.size()){
...
double ARA_Analysis::getWeightedVariance(vector<double> v, vector<double> weights){
  if (v.size() != weights.size()){
...
double ARA_Analysis::getWeightedRms(vector<double> v, vector<double> weights){
  double mean = getWeightedMean(v, weights);
  double sumOfSquares = 0.0;
  double sumOfWeights = accumulate(weights.begin(), weights.end(), 0.0);
  for (int i = 0; i < v.size(); i++){
...
double ARA_Analysis::getWeightedRmsPol(vector<vector<vector<double> > > delays, vector<vector<vector<double> > > weights, int polarization){
...
vector<vector<vector<double> > > ARA_Analysis::getVVVToNPower(vector<vector<vector<double> > > vvvIn, double power){
...
vector<vector<vector<double> > > ARA_Analysis::getHitTimeMagnitude_Lesser(vector<double> hitTimes, vector<double> magnitude, vector<vector<vector<vector<int> > > > Pairs){
...
bool ARA_Analysis::isUsablePattern(vector<double> magnitudes, double threshold){
  bool isUsable = false;
  int counter = 0;
...
double ARA_Analysis::getWeightedSum_Threshold(vector<double> v, vector<double> weights, vector<double> magnitudes, double threshold){
  if (v.size() != weights.size()){
    cerr << "ERROR: getWeightedSum(): value vector not same size as weight vector" << endl;
    return 0.0;
  }
...
double ARA_Analysis::accumulateWeights_Threshold(vector<double> weights, vector<double> magnitudes, double threshold){
  double sum = 0.0;
...
double ARA_Analysis::getWeightedMean_Threshold(vector<double> v, vector<double> weights, vector<double> magnitudes, double threshold){
  if (v.size() != weights.size()){
    cerr << "ERROR: getWeightedMean(): value vector not same size as weight vector" << endl;
    return 0.0;
  }
...
void ARA_Analysis::getWeightedVariance_Threshold(vector<double> v, vector<double> weights, vector<double> magnitudes, double threshold, double &sumOfSquares_out, double &sumOfWeights_out){
...
double ARA_Analysis::getWeightedRmsPol_Threshold(int polarization, vector<vector<vector<double> > > delays, vector<vector<vector<double> > > weights, vector<vector<vector<double> > > magnitudes,  double threshold ){
...
double ARA_Analysis::getLeastDifferenceFromTetrad(vector<vector<vector<double> > > delays)
{
...
int ARA_Analysis::getHitsUnderTimeFromTetrad(int polarization, double timeLimit, vector<vector<vector<double> > > delays)
{
  int hits = 0;
  for (int i = 0; i < delays.size(); i++){
    //    for (int j = 0; j < delays[i].size(); j++){
...
int ARA_Analysis::getHitsUnderTimeFromTetrad(int polarization, double timeLimit, vector<vector<vector<double> > > delays, vector<vector<vector<double> > > weights)
{
  int hits = 0;
  for (int i = 0; i < delays.size(); i++){
    //    for (int j = 0; j < delays[i].size(); j++){
...
vector<vector<vector<double> > > ARA_Analysis::getHitTimeDifferencesFromTetrad(vector<double> hitTimes, vector<double> peakIntPowers, vector<vector<vector<vector<int> > > > Pairs){
...
vector<double> ARA_Analysis::getVSigmas(vector<double> peaks, vector<double> averages, vector<double> variances){
...
vector<vector<vector<double> > > ARA_Analysis::getVVVSigmas(vector<double> vSigmas, vector<vector<vector<vector<int> > > > Pairs){
...
double ARA_Analysis::getLesser(double input1, double input2){
...
void ARA_Analysis::getMinimum(int length, double *array, int &index, double &minimum){ 
  double min = 1.E100;
  int minIndex = -1;
...
double* ARA_Analysis::getCorrelation_NoNorm(int length, double *oldY1, double *oldY2)
{
...
TGraph *ARA_Analysis::getCorrelationGraph_WFweight(TGraph *gr1, TGraph *gr2) {
...
vector<TGraph*> ARA_Analysis::getCorrelationGraphs_wBestTimes(vector<TGraph*> grIn, vector<vector< int > > pairs, vector<double> &bestTimes, vector<double> &bestCorrs){
...
vector<TGraph*> ARA_Analysis::getCorrelationGraphs(vector<TGraph*> grIn, vector<vector< int > > pairs ){
...
vector<TGraph*> ARA_Analysis::getHilbertGraphs_wBestTimes(vector<TGraph*> grIn, vector<double> &bestTimes, vector<double> &bestCorrs){
...
vector<TGraph*> ARA_Analysis::getHilbertCorrelationGraphs_wBestTimes(vector<TGraph*> grIn, vector<vector< int > > pairs, vector<double> &bestTimes, vector<double> &bestCorrs){
...
void ARA_Analysis::setupCorrelationPairs(int StationID, vector<vector<int> > &pairs_V, vector<vector<int> > &pairs_H){
...
vector<double> ARA_Analysis::getRms_Corr(vector<double> BestTimes, vector<vector<int> > pairs_from_correlation, vector<vector<vector<vector<int> > > > pairs_from_faces, vector<vector<double> > ant_loc){
...
vector<double> ARA_Analysis::getRms_Corr_Threshold(vector<double> BestTimes, vector<vector<int> > pairs_from_correlation, vector<vector<vector<vector<int> > > > pairs_from_faces, vector<vector<double> > ant_loc, vector<double> BestCorrs, double CorrThreshold){
...
double  ARA_Analysis::getPolarizationRatio(vector<TGraph*> waveforms, vector<int> polarizations){
...
void ARA_Analysis::getThirdVPeakOverRMS(vector<double> vPeakOverRms, vector<int> polarizations, vector<double> &ThirdVpeakOverRms){
...
double ARA_Analysis::getRms_Remove1(vector<double> v){
...
double ARA_Analysis::getRms_Remove1(vector<double> v, int &dropped_pair){
  double lowestRMS = 1.0E5;
  /*
    for(int i = 0; i < v.size(); i++){
      cout << v[i] << " : ";
...
vector<double> ARA_Analysis::getRms_Corr_Regular_Threshold_Remove1(vector<double> BestTimes, vector<vector<int> > pairs_from_correlation, vector<vector<vector<vector<int> > > > pairs_from_faces, vector<vector<double> > ant_loc, vector<double> BestCorrs, double CorrThreshold){
...
vector<double> ARA_Analysis::getRms_Corr_Regular_Threshold_Remove1(vector<double> BestTimes, vector<vector<int> > pairs_from_correlation, vector<vector<vector<vector<int> > > > pairs_from_faces, vector<vector<double> > ant_loc, vector<double> BestCorrs, double CorrThreshold, vector<int> &dropped_pairs){
...
vector<vector<vector<vector<int> > > > ARA_Analysis::setupFaces(int stationId){
...
vector<double> ARA_Analysis::getRms_Faces(
...
bool ARA_Analysis::isCutOnFaceError(int nFaces, double* faceCuts, vector<double> faceErrors)
{
  bool isCut = true;
...
vector<double> ARA_Analysis::getRms_Faces_Thresh(
...
void ARA_Analysis::getAbsMaximum_N(TGraph* plot, int nPeaks, double timeApart, vector<double> &xs, vector<double> &ys)
{
  xs.clear();
  ys.clear();
  ...
  for (int iPeak = 0; iPeak < nPeaks; iPeak++){ //loop over peaks
      y_good = -9.0E99;
      y_upper = 9.0E99;
      if (iPeak > 0){
		   y_upper = ys[iPeak-1];
      }
      for (int i = 0; i < nPoints; i++){ //loop over waveform
		test = plot->GetPoint(i, x_temp, y_temp);
		if (iPeak == 0){ //if you're looking for your first peak
	  		if (y_temp > y_good){ //if this point is bigger, set it to the standard
	    		x_good = x_temp;
	    		y_good = y_temp;
	  		}
		} 
		else { //if you're looking for a subsequent peak
	  		for (int iTimeTest = 0; iTimeTest < xs.size(); iTimeTest++){
	    		if (y_temp > y_good && y_temp < y_upper && abs(x_temp - xs[iTimeTest]) > timeApart){ //make sure the second peak is far enough away
	      			x_good = x_temp;
	   			   y_good = y_temp;
	    		}
			  }
      ...
      xs.push_back(x_good); //store the result
      ys.push_back(y_good);
...
void ARA_Analysis::getAbsMaximum_N(vector<TGraph*> graphs, int nPeaks, double timeApart, vector<vector<double> > &xs, vector<vector<double> > &ys){
...
vector<double> ARA_Analysis::getRms_Faces_Thresh_N(
...
double ARA_Analysis::getPhase(FFTWComplex &theNum){
  return atan2(theNum.im, theNum.re);
}

TGraph * ARA_Analysis::makeRawPhase(TGraph *grWave) {
...
TGraph * ARA_Analysis::getGraphDifference(TGraph *gr1, TGraph* gr2){
...
TGraph * ARA_Analysis::getGraphSum(TGraph *gr1, TGraph* gr2){
...
TGraph * ARA_Analysis::makeSpectrum_mVPerRootHz(TGraph *grWave){
...
void ARA_Analysis::getWavefrontRMS(vector<TGraph*> graphs_raw, vector<double> waveformRMS, 
				   int stationID, vector< vector<double> > ant_loc,
				   double threshold,
				   double &wavefrontRMS_V, double &wavefrontRMS_H,
				   double interpolationTimeStep = 0.625,
				   double integrationTime = 5.0,
				   int numSearchPeaks=2,
				   double peakSeparation=5.0
				   ){
  //  double interpolationTimeStep = 0.625;
  int numBinsToIntegrate = (int)(integrationTime/interpolationTimeStep);
  ...
  vector<TGraph*> grWaveformsInt = makeInterpolatedGraphs(graphs_raw, interpolationTimeStep, yLabel, xLabel, titlesForGraphs);
...
void ARA_Analysis::getWavefrontRMSVector(vector<TGraph*> graphs_raw, vector<double> waveformRMS, 
					 int stationID, vector< vector<double> > ant_loc,
  ...
  for (int i = 0; i < threshold.size(); i++){
    vector<double> rms_faces_V = getRms_Faces_Thresh_N(vvHitTimes, vvRMS_10overRMS, threshold[i], 0, faces, ant_loc);
    vector<double> rms_faces_H = getRms_Faces_Thresh_N(vvHitTimes, vvRMS_10overRMS, threshold[i], 1, faces, ant_loc);
    
    sort(rms_faces_V.begin(), rms_faces_V.end());
    sort(rms_faces_H.begin(), rms_faces_H.end());
    
    vWavefrontRMS_V.push_back(log10(rms_faces_V[0]));
    vWavefrontRMS_H.push_back(log10(rms_faces_H[0]));
  }
...
void ARA_Analysis::saveGraph(TGraph* gr1, string filename){
...
void ARA_Analysis::saveHist(TH1* hist, string filename){
...
void ARA_Analysis::saveHistWithFit(TH1* hist, TF1* myFit, string filename){
...
void ARA_Analysis::save2Graphs(TGraph* gr1, TGraph* gr2, string filename){
...
void ARA_Analysis::save16Graphs(vector<TGraph*> grs, string filename){
...
void ARA_Analysis::saveNGraphs(vector<TGraph*> grs, string filename, int numGraphs){
...
void ARA_Analysis::saveNGraphs_2Tlines(vector<TGraph*> grs, string filename, int numGraphs, vector<double> averages, vector<double> deviations){
...
vector<TGraph*> ARA_Analysis::makeGraphsFromRF(UsefulAtriStationEvent* realAtriEvPtr, int numGraphs, string xlabel, string ylabel, vector<string> titles){
...
void ARA_Analysis::deleteGraphVector(vector<TGraph*> graphs){
  for (int i=0; i < graphs.size(); i++){
    delete graphs[i];
  }
  graphs.clear();
...
vector<TGraph*> ARA_Analysis::makeInterpolatedGraphs(vector<TGraph*> graphsIn, double intTimestep, string xlabel, string ylabel, vector<string> titles){
...
vector<TGraph*> ARA_Analysis::makePaddedGraphs(vector<TGraph*> graphsIn, bool isSoftTrigger, string xlabel, string ylabel, vector<string> titles){
...
vector<TGraph*> ARA_Analysis::makePowerSpectrumGraphs(vector<TGraph*> graphsIn, string xlabel, string ylabel, vector<string> titles){
...
vector<TGraph*> ARA_Analysis::makeVoltageSpectrumGraphs(vector<TGraph*> graphsIn, string xlabel, string ylabel, vector<string> titles){
...
vector<TGraph*> ARA_Analysis::makePhaseGraphs(vector<TGraph*> graphsIn, string xlabel, string ylabel, vector<string> titles){
...
void ARA_Analysis::saveHistOverlap(vector<TH1D*> hists, string filename){
  gStyle->SetOptStat(0);
...
void ARA_Analysis::setHistLineColors(vector<TH1D*> hists){
  for (int i = 0; i < hists.size(); i++){
    if (i == 0){
      hists[i]->SetLineColor(kBlack);
    }
...
void ARA_Analysis::saveHistOverlapPolarized(vector<TH1D*> histsV, vector<TH1D*> histsH, string filename, int logOpt){
...
void ARA_Analysis::deleteHistVector(vector<TH1D*> hists){
  for (int i=0; i < hists.size(); i++){
    delete hists[i];
  }
  hists.clear();
...
vector<TH1D*> ARA_Analysis::initializeHistVector(int size, string name, int bins, double lowBound, double upperBound){
  vector<TH1D*> vHist;
  stringstream ss;
...
int ARA_Analysis::getRunNum(char* runfile){
...
void ARA_Analysis::saveHist2D(TH2* hist, string filename){
...
void ARA_Analysis::saveHist2D(TH2* hist, string filename, int logOpt){
...
vector<TH2D*> ARA_Analysis::initializeHistVector2D(int size, string name, int bins_x, double lowBound_x, double upperBound_x, int bins_y, double lowBound_y, double upperBound_y){
  vector<TH2D*> vHist;
  stringstream ss;
...
void ARA_Analysis::saveNHists2D(vector<TH2D*> hists, string filename_base){
...
string ARA_Analysis::getProcessedFilename(int stationID, char* outputdir, char* runfile ){
  
  stringstream ss;
  int runNum = getrunNum(runfile);
  ss.str("");
  ss << outputdir << "/processed_station_" << stationID << "_run_" << runNum << ".root";

  return ss.str();
}

string ARA_Analysis::getProcessedFilename_filter(int stationID, char* outputdir, char* runfile ){
...
string ARA_Analysis::getProcessedFilename_recoRadius(int stationID, char* outputdir, char* runfile, int radius){
...
string ARA_Analysis::getNoiseFilename(int stationID, char* outputdir, char* runfile ){
...
string ARA_Analysis::getRunSummaryFilename(int stationID, char* outputdir, char* runfile ){
  
  stringstream ss;
  int runNum = getrunNum(runfile);
  ss.str("");
  ss << outputdir << "/run_summary_station_" << stationID << "_run_" << runNum << ".root";

  return ss.str();
}

vector<TGraph*> ARA_Analysis::initializeGraphVector(int size){
  vector<TGraph*> vGr;
...
vector<TGraph*> ARA_Analysis::makeDifferenceGraphs(vector<TGraph*> graphsIn, int graphBegin, int graphEnd, string xlabel, string ylabel, vector<string> titles){
...
vector<TGraph*> ARA_Analysis::makeSumGraphs(vector<TGraph*> graphsIn, int graphBegin, int graphEnd, string xlabel, string ylabel, vector<string> titles){
